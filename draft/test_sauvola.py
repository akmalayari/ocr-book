"""
test_sauvola.py — Test de la binarisation Sauvola (scikit-image).

Requiert : pip install scikit-image
...
Sorties : output/sauvola_test/

Usage :
    python draft/test_sauvola.py
    python draft/test_sauvola.py --ocr
    python draft/test_sauvola.py --ocr baseline sauvola_25_02
    python draft/test_sauvola.py --compare
    python draft/test_sauvola.py --pages page_5 page_6
"""
...
OUT_DIR    = Path(__file__).parent.parent / "output" / "sauvola_test"

DEFAULT_PAGES = ["page_1", "page_2", "page_5", "page_6"]

# (window_size, k, blur_before)
# blur_before=True : applique GaussianBlur(5,5) avant Sauvola
SAUVOLA_CONFIGS: dict[str, tuple[int, float, bool] | None] = {
    "baseline":          None,   # GaussianBlur(5,5) + adaptive(31, 15)
    "sauvola_25_02":     (25,  0.2, False),
    "sauvola_51_02":     (51,  0.2, False),
    "sauvola_25_03":     (25,  0.3, False),
    "sauvola_51_03":     (51,  0.3, False),
    "blur_sauvola_25_02": (25, 0.2, True),
    "blur_sauvola_51_02": (51, 0.2, True),
}
...
def _binarize_sauvola(gray: np.ndarray, window_size: int, k: float, blur: bool) -> np.ndarray:
    src = cv2.GaussianBlur(gray, (5, 5), 0.0) if blur else gray
    thresh = threshold_sauvola(src, window_size=window_size, k=k)
...
def _get_image(img_path: Path, config_name: str) -> np.ndarray:
    ...
    cfg  = SAUVOLA_CONFIGS[config_name]
    if cfg is None:
        return _binarize_adaptive(gray)
    return _binarize_sauvola(gray, *cfg)
...
def phase1_visualize(images: list[Path]) -> None:
    ...
    for img_path in images:
        ...
        steps: list[tuple[str, np.ndarray]] = [
            ("01_gray",      gray),
            ("02_blurred",   blurred),
            ("03_baseline",  _binarize_adaptive(gray)),
        ]
        for config_name, cfg in SAUVOLA_CONFIGS.items():
            if cfg is None:
                continue
            window_size, k, blur = cfg
            src = blurred if blur else gray
            steps.append((f"sauvola_{config_name}", _binarize_sauvola(gray, window_size, k, blur)))
    ...
    print(f"\nImages dans {OUT_DIR}")
    print("Pour lancer l'OCR : --ocr  (ou --ocr baseline sauvola_25_02)")
...
def phase2_ocr(images: list[Path], configs_to_run: list[str]) -> None:
    ...
    for img_path in images:
        for config_name in configs_to_run:
            if config_name not in SAUVOLA_CONFIGS:
...
def phase3_compare(images: list[Path]) -> None:
    ...
    for img_path in images:
        ...
        for config_name in SAUVOLA_CONFIGS:
...
def main() -> None:
    ...
    parser.add_argument("--ocr", nargs="*", metavar="CONFIG",
                        help="Configs OCR. Sans valeur = toutes. Ex: baseline sauvola_25_02")
    ...
    configs_to_run = list(SAUVOLA_CONFIGS.keys()) if len(args.ocr) == 0 else args.ocr
