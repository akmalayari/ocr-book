...
| **Intra-page parallelism patch** (`apply_paddlex_patch_parallel.py`) | Retained and installed automatically; default 1, measured ~30% gain at 3 on the reference machine |
...
**Intra-page parallelization (global ThreadPoolExecutor pool, 2026-04-09)** :
**retained**, `docs/dev/apply_paddlex_patch_parallel.py`. The measurements below
are historical results from the Ryzen AI 9 HX370 / Radeon 890M test machine, not
...
Gain on that machine: ~35 min on 150 pages. The tested configuration was
`-np 3 -c 6144` (2048/slot). Reduction from `-c 12288` to `-c 6144` added
~2.5s/page without observed truncation.
...
**Conclusion for that APU:** intra-page continuous batching with one server was
the only effective optimization. The stable value was `-np 3`; other hardware
must start at the project default of 1 and validate 2 before going higher.

**Best tested configuration (2026-04-09, reference machine):** 1 server,
`-np 3`, `-c 6144`. Measured speed on `photos/test/`: **43.4s/page text,
37s/page with graph**.
...
These are tested values, not current defaults. The parallel patch now reads the
runtime value, so changing `OCR_N_PARALLEL` does not require reapplying it.
